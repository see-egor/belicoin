...
// https://belicointalk.org/index.php?topic=182430.msg1904506#msg1904506
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	// vMerkleTree: 5a2e19825b
        ...
        // Genesis block
        const char* pszTimestamp = " 1535117381";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Litecoind: Reduce the exempted free transactions to 500 bytes (from belicoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("belicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("belicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadbelicoinMiner(void* parg);

static bool fGeneratebelicoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static belicoinMiner(CWallet *pwallet)
{
    printf("belicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("belicoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratebelicoins)
    {
        if (fShutdown)
            return;
        ...
        printf("Running belicoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadbelicoinMiner(void* parg)
{
    ...
    try
    {
        ...
        belicoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadbelicoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadbelicoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadbelicoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatebelicoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratebelicoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratebelicoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d belicoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadbelicoinMiner, pwallet))
                printf("Error: CreateThread(ThreadbelicoinMiner) failed\n");
            Sleep(10);
        }
    }
}
