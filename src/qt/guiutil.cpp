#include "guiutil.h"
#include "belicoinaddressvalidator.h"
...
{
    widget->setMaxLength(belicoinAddressValidator::MaxAddressLength);
    widget->setValidator(new belicoinAddressValidator(parent));
    widget->setFont(belicoinAddressFont());
}
...
bool parsebelicoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    if(uri.scheme() != QString("smallchange"))
        return false;

    // check if the address is valid
    CbelicoinAddress addressFromUri(uri.path().toStdString());
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!belicoinUnits::parse(belicoinUnits::BLI, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsebelicoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parsebelicoinURI(uriInstance, out);
}
