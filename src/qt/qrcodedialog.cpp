...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BLI")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BLI unit input in lnReqAmount, we generate the URI with BLI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(belicoinUnits::format(belicoinUnits::BLI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
